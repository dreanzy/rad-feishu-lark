...
export type WorkspaceListData = {
	key: string;
	items: Array<{ path: string; label: string; isCurrent: boolean }>;
};
...
export function buildResumeCard(data: ResumeSessionPage) {
	const scopeLabel =
		data.scope === "current"
			? msg("card.resume.scope.current")
			: msg("card.resume.scope.all");
	const elements: any[] = [
		{
			tag: "markdown",
			content: [
	...
	for (const item of data.items) {
		...
		if (item.workspaceLabel)
			lines.push(
				`${msg("card.resume.workspace_label")}${escapeMarkdown(item.workspaceLabel)}`,
			);
		elements.push({
			tag: "markdown",
			content: lines.join("\n"),
			...
			actions: [
				{
					...
					text: {
						...
						content: item.isCurrent
							? msg("card.resume.current_session")
							: msg("card.resume.switch_to"),
					},
...
export function buildWorkspaceListCard(data: WorkspaceListData) {
	const elements: any[] = [];

	if (!data.items.length) {
		elements.push({
			tag: "div",
			text: { tag: "plain_text", content: msg("card.workspace_list.empty") },
		});
		return {
			config: sharedCardConfig(),
			header: {
				template: "indigo",
				title: { tag: "plain_text", content: msg("card.workspace_list.title") },
			},
			elements,
		};
	}
	...
	for (const item of data.items) {
		const label = item.isCurrent
			? `${item.label}${msg("card.workspace_list.current_badge")}`
			: item.label;
		elements.push({
			tag: "div",
			text: {
				tag: "plain_text",
				content: `${msg("card.workspace_list.workspace_label")}${item.path}`,
			},
		});
		elements.push({
			tag: "action",
			actions: [
				{
					...
					type: item.isCurrent ? "primary" : "default",
					value: {
						action: "pi_feishu_workspace_select",
						key: data.key,
						path: item.path,
					},
				},
	...
	elements.push({
		tag: "note",
		elements: [{ tag: "plain_text", content: msg("card.workspace_list.hint") }],
	});
...
export function parseModelActionValue(
	value: unknown,
): { key: string; provider: string; modelId: string } | undefined {
...
export function parseResumePageActionValue(
	value: unknown,
): { key: string; scope: ResumeScope; page: number } | undefined {
	if (!value || typeof value !== "object") return undefined;
	const raw = value as any;
...
export function parseResumeSelectActionValue(
	value: unknown,
):
	| { key: string; scope: ResumeScope; page: number; sessionPath: string }
	| undefined {
	if (!value || typeof value !== "object") return undefined;
	const raw = value as any;
	if (raw.action !== "pi_feishu_resume_select") return undefined;
	if (typeof raw.key !== "string" || typeof raw.sessionPath !== "string")
		return undefined;
	if (raw.scope !== "current" && raw.scope !== "all") return undefined;
	if (typeof raw.page !== "number" || !Number.isFinite(raw.page))
		return undefined;
...
export function parseWorkspaceSelectActionValue(
	value: unknown,
): { key: string; path: string } | undefined {
	if (!value || typeof value !== "object") return undefined;
	const raw = value as any;
	if (raw.action !== "pi_feishu_workspace_select") return undefined;
	if (typeof raw.key !== "string" || typeof raw.path !== "string")
		return undefined;
	return { key: raw.key, path: raw.path };
}

function buildResumeScopeButton(
	key: string,
	scope: ResumeScope,
	active: boolean,
) {
