...
export function pruneRecentMap(
...
export function conversationKey(message: FeishuMessage) {
	...
	if (message.chatMode === "topic")
		return `group:${message.chatId}:thread:${message.messageId}`;
...
export function parseMessageInput(
	...
	try {
		...
		if (
			...
			attachments.push({
				...
				fileName:
					typeof json.file_name === "string" ? json.file_name : undefined,
			});
			collectAttachments(json, attachments);
			return { text: "", attachments };
		}
	...
	return {
		text: message.msgType === "text" ? message.content : `[${message.msgType}]`,
		attachments,
	};
}
...
function extractPostText(
	...
	if (Array.isArray(node))
		return node.map((item) => extractPostText(item, attachments)).join("");
...
function collectAttachments(value: unknown, attachments: FeishuAttachment[]) {
	const seen = new Set(
		attachments.map((item) => `${item.kind}:${item.fileKey}`),
	);
	walk(value);
